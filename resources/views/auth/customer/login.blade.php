...
<div class="bg-gray-100 flex items-center justify-center h-screen">
    <div class="bg-white p-8 rounded shadow-md flex w-full max-w-3xl">
        <!-- Left Side (Image) -->
        <div class="w-1/2">
            <img src="{{ asset('public/image/customer-login.jpg') }}" alt="Login Image" class="w-full h-full object-cover rounded">
        </div>
...
<!--             <div class="p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
  <span class="font-medium">Success alert!</span> Change a few things up and try submitting again.
            ...
            <form action="{{ route('customerlogin') }}" method="POST">
                ...
                <button type="submit" class="w-full bg-red-500 text-white py-2 px-4 rounded hover:bg-red-600">Login</button>
                <a href="{{route('password.forgot')}}"><p class="block text-sm font-small text-gray-500 mt-4">Forgot Password ?</p></a>
                <a href="" id="otpLink"><p class="block text-sm font-small text-gray-500 mt-4" onclick="updateLink()">Login Using OTP </p></a>
...
<script>
    function updateLink() {
        ...
        var baseUrl = "{{ route('customerotplogin') }}";

        // Update the href of the link with the email as a query parameter
        document.getElementById('otpLink').href = baseUrl + '?email=' + encodeURIComponent(email);
    }
</script>
